...
*/
    ...
    public void onAttachedToRecyclerView(RecyclerView recyclerView) {
        ...
        xRecyclerView = recyclerView instanceof XRecyclerView ? (XRecyclerView) recyclerView : null;
        if (manager instanceof GridLayoutManager) {//对GridLayoutManager做特殊处理
            final GridLayoutManager gridManager = ((GridLayoutManager) manager);
            gridManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
                @Override
                public int getSpanSize(int position) {
                    if (xRecyclerView == null) {//说明是普通的RecyclerView布局
                        return getGridSpanSize(position);
                    } else {//说明是XRecyclerView布局，XRecyclerView有头和尾需要进一步做特殊处理
                        return xRecyclerView.isHeaderFooter(position) ? gridManager.getSpanCount() :
                                getGridSpanSize(position - (xRecyclerView.getHeadersCount() + 1));
                    }
                }
    ...
    @Override
    public void onBindViewHolder(final HelperRecyclerViewHolder holder, int position) {
        int type = judgeType(position);
    ...
    @Override
    public void onBindViewHolder(HelperRecyclerViewHolder holder, int position, List<Object> payloads) {
    ...
    public abstract void onBindChildViewHolder(HelperRecyclerViewHolder holder,
    ...
    public interface OnHeaderClickListener<T> {
        void onHeaderClick(GroupedRecyclerViewAdapter adapter, HelperRecyclerViewHolder holder, int groupPosition, T item);
    }

    public interface OnFooterClickListener<T> {
        void onFooterClick(GroupedRecyclerViewAdapter adapter, HelperRecyclerViewHolder holder, int groupPosition, T item);
    }

    public interface OnChildClickListener<T> {
        void onChildClick(GroupedRecyclerViewAdapter adapter, HelperRecyclerViewHolder holder,
                          int groupPosition, int childPosition, T item);
    }
}